...
use crate::models::transaction::{CreateTransaction, CreateEntry};
...
pub fn validate_entries(entries: &[CreateEntry]) -> ApiResult<()> {
    if entries.is_empty() {
        return Err(ApiError::Validation("Transaction must have at least one entry".to_string()));
    }
    ...
    for (idx, entry) in entries.iter().enumerate() {
            ...
            (Some(debit), None) => {
                if debit <= Decimal::ZERO {
                    return Err(ApiError::Validation(
                        format!("Entry {} debit amount must be positive", idx)
                    ));
                }
            }
            (None, Some(credit)) => {
                if credit <= Decimal::ZERO {
                    return Err(ApiError::Validation(
                        format!("Entry {} credit amount must be positive", idx)
                    ));
                }
            }
            (Some(_), Some(_)) => {
                return Err(ApiError::Validation(
                    format!("Entry {} cannot have both debit and credit", idx)
                ));
            }
            (None, None) => {
                return Err(ApiError::Validation(
                    format!("Entry {} must have either debit or credit", idx)
                ));
            }
        }
        ...
        if entry.currency_code.is_empty() {
            return Err(ApiError::Validation(
                format!("Entry {} must have a currency code", idx)
            ));
        }
        
        if entry.currency_code.len() != 3 {
            return Err(ApiError::Validation(
                format!("Entry {} currency code must be exactly 3 characters", idx)
            ));
        }
    }
...
pub fn validate_double_entry_balance(entries: &[CreateEntry]) -> ApiResult<()> {
    ...
    if total_debits == Decimal::ZERO && total_credits == Decimal::ZERO {
        return Err(ApiError::Validation("Transaction has no amounts".to_string()));
    }
    
    Ok(())
}

pub async fn validate_currencies_exist(
    pool: &PgPool,
    entries: &[CreateEntry],
) -> ApiResult<()> {
    let unique_currencies: HashSet<&str> = entries
        .iter()
        .map(|e| e.currency_code.as_str())
        .collect();
    
    for currency_code in unique_currencies {
        get_currency_by_code(pool, currency_code).await
            .map_err(|_| ApiError::Validation(
                format!("Currency {} does not exist", currency_code)
            ))?;
    }
...
pub async fn validate_accounts_exist(
    ...
    let unique_accounts: HashSet<i64> = entries
        .iter()
        .map(|e| e.account_id)
        .collect();
    ...
    for account_id in unique_accounts {
        ...
        if account.ledger_id != ledger_id {
            return Err(ApiError::Validation(
                format!("Account {} does not belong to ledger {}", account_id, ledger_id)
            ));
        }
    }
...
pub async fn validate_account_versions(
    ...
    for entry in entries {
        if let Some(expected_version) = entry.account_version {
            let row = sqlx::query(
                r#"SELECT version FROM account WHERE id = $1 FOR UPDATE"#
            )
...
mod tests {
    ...
    fn test_validate_entries_invalid_currency_code() {
        ...
        let result = validate_entries(&entries);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("exactly 3 characters"));
    }
